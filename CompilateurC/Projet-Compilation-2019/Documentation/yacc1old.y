...
typedef struct SymboleList {
	...
	struct SymboleList *next;
}SymboleList;

struct Symbole *crerSymbole(char *nom,char*type){
	struct Symbole *newlist =(struct Symbole *) malloc(sizeof(struct Symbole));
	newlist->nom = nom;
	newlist->type = type;
	return newlist;
}

struct SymboleList *Append(struct Symbole *s1, struct SymboleList *sl){
	struct SymboleList *newlist =(struct SymboleList *) malloc(sizeof(struct SymboleList));
	newlist->symbole = s1;
	newlist->next = sl;
	return newlist;
}
struct SymboleList *Append2(struct SymboleList *s1, struct SymboleList *s2){
	struct SymboleList *s3=s1;
	while(s3->next != NULL){
		s3 = s3->next;
	}
	s3->next = s2;
...
struct Symbole *search(char* nom,struct SymboleList *sl){
	struct SymboleList *node = sl;

	while(node != NULL){
		if(node->symbole->nom == nom){
			return node->symbole;
		}
		else{
			node = node->next;
...
struct SymboleList *tableSymbole ;
...
void printSymbole(struct SymboleList *sl) {
	if (sl == NULL) return;
	struct Symbole *s1 = sl->symbole;
	printf("%s : %s \n",s1->nom,s1->type);
	printSymbole(sl->next);
}
...
%left OP
%left REL
%type<ast> variable_tab selection_switch tableau0 GEQ LEQ EQ NEQ NOT EXTERN BREAK RETURN PLUS MUL DIV LSHIFT RSHIFT BAND BOR LAND LOR LT GT  MOINS CONSTANTE fonction liste_parms parm liste_instructions instruction iteration selection saut affectation bloc appel variable condition expression liste_expressions binary_rel binary_comp
%type<str> type IDENTIFICATEUR
%type<list> liste_fonctions
%type<sl> liste_declarations declaration liste_declarateurs
%type<symbole> declarateur
%start programme
%%
programme :	
		liste_declarations liste_fonctions {tableSymbole =  Append2(tableSymbole,$1);listprogramme = $2; }
;
liste_declarations :	
		liste_declarations declaration {$$ = Append2($2,$1);}
	|	{$$=NULL;}
;
liste_fonctions :	
...
;
declaration :	
		type liste_declarateurs ';' {struct SymboleList *node = $2;
									while($2 != NULL){
										$2->symbole->type = $1;
										$2 = $2->next;
									}
...
;
liste_declarateurs :	
		liste_declarateurs ',' declarateur {$$ = Append($3, $1);}
	|	declarateur {$$ = Append($1,NULL);}
;
declarateur	:	
		IDENTIFICATEUR {$$ = crerSymbole($1,"NULL");}
	|	IDENTIFICATEUR tableau0 {$$ = crerSymbole($1,"NULL");}
;
tableau0 :
		tableau0 '[' CONSTANTE ']' { }
	|	'[' CONSTANTE ']' {}
...
asprintf(&buffer, "[label=\"%s, %s\" shape=invtrapezium color=blue]", $2, $1);
	...
	|	EXTERN type IDENTIFICATEUR '(' liste_parms ')' ';' {struct Symbole *s = crerSymbole($3,$2);tableSymbole = Append(s,tableSymbole); }
;
type :	
		VOID {$$="VOID";}
	|	INT {$$="INT";}
...
;
bloc :	
		'{' liste_declarations liste_instructions '}' {tableSymbole = Append2($2,tableSymbole);  $$ =creation_noeud(yylineno,"[label=\"BLOC\"]", none, none,none,  1 ,$3);$$->name = nextname();}
;
appel :	
